    ...
    def __init__(self, root, email):
        ...
        def getans():
            if ques.get() == 'What if I exceed my free baggage weight allowance':
                messagebox.askokcancel("Query Result","Customers carrying more than exceeded baggage limit will be charged the folllowing excess baggage fees:\n Domestic: INR 500/kg\nInternational: INR 525/kg")
            elif ques.get() == 'What item are not permitted in the flights':
                messagebox.askokcancel("Query Result","Prohibited in checked and cabin baggage:\nAcids,\nPoisons,\nToxic,\nFlammable Liquids,\nExplosives,\nBleach,\netc")
                ...
                messagebox.askokcancel("Query Result","Refund for the cancelled tickets will be processed after the deduction of appropriate cancellation fees,which may take up to 7 working days in your registered bank account")
            elif ques.get() == 'Will I be contacted in advance if my flight is delayed, pre-poned or cancelled?':
                messagebox.askokcancel("Query Result","Yes you'll recieve a mail and a message on your registered email id and contact number")
            elif ques.get() == 'Do I need to book a separate seat for an infant?':
                messagebox.askokcancel("Query Result","For safety reasons, children above the age of seven 7 days and under the age of two 2 years, as on the date of travel, can travel as Infants. Age proof needs to be provided at the time of check-in and no extra bookings is required for them")
            elif ques.get() == 'How do I book snacks for my flight online and what are the charges?':
                messagebox.askokcancel("Query Result","You may pre-book the snacks at the time of booking pr add it later either by visiting my bookings section or by calling our contact center")
            elif ques.get() == 'What items are considered as special baggage and how can I carry them?':
                messagebox.askokcancel("Query Result","Following are considered as special baggage:\nSports/skiing equipment,\ngolf bags,\nbicycles,\nLCD/LED TVs,\nany other large or add size items.\n\n Following charges are applied:\nDomestic: INR 1200/bag\nInterational: INR 2500/bag")
            elif ques.get() == 'Who can I contact for feedbacks and comments?':
                messagebox.askokcancel("Query Result","You can visit 'contact us' page and mail us your feedbacks on the provided email")
